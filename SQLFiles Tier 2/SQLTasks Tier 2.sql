...
select max(m.firstname), max(m.surname) from `Members` as m,`Bookings` as b 
where m.memid=b.memid group by b.memid
...
select max(m.firstname), max(m.surname), max(f.name)
from `Bookings` as b
